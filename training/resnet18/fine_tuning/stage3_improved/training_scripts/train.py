"""
Project 2 — Fine-tuning STAGE 3 (IMPROVED) — class-balanced fine-tuning, Option A.

This is a copy of fine_tuning/stage3_323/training_scripts/train.py with EXACTLY
one behavioral change: a weighted cross-entropy loss with a single tunable knob
(--balance_strength). strength=0.0 reproduces the original unweighted stage 3.
All outputs are routed through --run_name under fine_tuning/stage3_improved/;
the frozen stage3_323/ baseline is never written to (hard write-guard in Cell 2).
See Cell 7b (weights) and Cell 19b (over-correction guard). ~323 real frames:
30 manual + game4 + game5 PGN.

# FINE-TUNING (NOT ZERO-SHOT). 30 manual-labelled real images from games
# 8-11 + the FULL game4 and game5 PGN frames are in training. game7 is
# the monitor and gates checkpoint selection. Pretrained weights:
# zero_shot/results/best_synth.pt (v1 baseline, peak game7 real_val
# 0.5923) — NOT zero_shot_v1.5, NOT stage1_10/stage2_30 weights.

Two-phase recipe (Wölflein 2021, §4.3):
  Phase A (epochs 1-5)   — freeze backbone, train fc only @ lr=1e-3.
  Phase B (epochs 6-30)  — unfreeze all, lr=1e-4. NO scheduler.
Augmentation: same as stage 2 (color jitter @0.7, shear @0.8 ±8°, noise
@0.5 std=0.015), applied to BOTH manual and PGN training samples.
Sampler: shuffle=True (natural PGN class distribution preserved on
purpose). Checkpoint selection: real_val_acc on game7. Early stop
patience=8.

Stage 3 / 5 (Comparison B alongside stage 5). Cold-start from v1
baseline weights. Single experimental variable vs. stage 2: training
data expanded from 30 manual labels to 30 manual + game4 PGN + game5
PGN (~323 real frames total). Test partition is RESTRICTED to games
2/6 since games 4 and 5 are now training data.

Test partition:
   monitor: game7 (55 frames);
   held-out test: games 2/6 ONLY (~169 frames / ~10,816 squares);
   NOT comparable to stages 1, 2 which tested on games 2/4/5/6 —
   see stage-2 reference re-eval at end of run for cross-partition bridge.
"""
...
# --- CLI (stage3_improved). Defaults reproduce the original stage-3 unweighted
# run exactly (balance_strength=0.0 → all-ones weight → plain CE). parse_known_args
# so the script still runs cell-by-cell under Jupyter (which injects -f <kernel>).
def _parse_args():
    p = argparse.ArgumentParser(
        description="Stage 3 (improved) — class-balanced fine-tuning (Option A).")
    p.add_argument("--run_name", type=str, default="debug",
                   help="output subdir under stage3_improved/{checkpoints,results,plots}/")
    p.add_argument("--balance_strength", type=float, default=0.0,
                   help="0.0 = plain CE (== original stage 3); 1.0 = full weighting.")
    p.add_argument("--balance_formula", type=str, default="inv_sqrt",
                   choices=["inv_sqrt", "effective_num"],
                   help="base weight formula before strength interpolation.")
    p.add_argument("--beta", type=float, default=0.999,
                   help="effective-number-of-samples beta (only for effective_num).")
    p.add_argument("--l2sp_lambda", type=float, default=0.0,
                   help="L2-SP weight-anchoring strength: lambda * sum_backbone ||theta - theta_0||^2 "
                        "added to the training loss in Phase B (theta_0 = loaded v1 synth init). "
                        "0.0 == disabled == byte-equivalent to the s00 baseline.")
    p.add_argument("--freeze_bn", action="store_true", default=False,
                   help="Freeze BatchNorm running stats (mean/var) during training: BN layers run in "
                        "eval mode so the synth-init running stats are kept and not updated, while BN "
                        "affine weight/bias stay trainable. Targets the forgetting mechanism (BN-stat "
                        "drift) that L2-SP-on-weights cannot reach. False == byte-equivalent to s00.")
    p.add_argument("--rehearsal_frac", type=float, default=0.0,
                   help="Synthetic-rehearsal: target fraction of each training batch drawn from v1 "
                        "synth TRAIN squares (dataset_v1 minus the 5%% monitor slice), via a "
                        "WeightedRandomSampler over ConcatDataset([real, synth]). Mirrors Stage 5's "
                        "exposure mechanism. Real exposure per epoch is held ~constant (num_samples = "
                        "n_real/(1-frac)). 0.0 == disabled == byte-equivalent to s00.")
...
BALANCE_FORMULA = ARGS.balance_formula
BALANCE_BETA = float(ARGS.beta)
...
print(f"[config] run_name={RUN_NAME}  balance_strength={BALANCE_STRENGTH}  "
      f"formula={BALANCE_FORMULA}  beta={BALANCE_BETA}  l2sp_lambda={L2SP_LAMBDA}  "
      f"freeze_bn={FREEZE_BN}  rehearsal_frac={REHEARSAL_FRAC}  seed={SEED}")
...
HELD_OUT_GAMES = [2, 6]  # games 4 and 5 are training data in stage 3
TRAIN_PGN_GAMES = [4, 5]  # full games used as training labels
...
# === Experiment dirs (stage3_improved — ALL outputs routed through RUN_NAME) ===
...
# --- HARD GUARD: never write into the frozen baseline. Every output dir must
# resolve under stage3_improved/ and must not name the frozen stage3 folders.
for _name, _d in [("CHECKPOINTS_DIR", CHECKPOINTS_DIR), ("RESULTS_DIR", RESULTS_DIR),
    ...
    assert _abs.startswith(os.path.realpath(EXP_DIR) + os.sep), (
        f"WRITE-GUARD: {_name}={_abs} is not under stage3_improved/ ({EXP_DIR}). Aborting.")
    assert "stage3_323" not in _abs and "/stage3/" not in (_abs + "/"), (
        f"WRITE-GUARD: {_name}={_abs} names a frozen-baseline path. Aborting.")
...
# --- Over-correction guard references (read-only) -----------------------------
# The frozen baseline's per-class accuracy on the easy/common classes. If the
# weighted run drives `empty` (or pawns) meaningfully below these, we have
# over-corrected — the trap. Loaded from the FROZEN stage3_323 results.
OVERCORRECTION_EMPTY_TOL = 0.01     # abs drop in `empty` acc that trips the alarm
_BASELINE_RESULTS_DIR = f"{PROJECT_ROOT}/fine_tuning/stage3_323/results"


def _load_baseline_per_class(json_name):
    p = Path(f"{_BASELINE_RESULTS_DIR}/{json_name}")
    if not p.exists():
        print(f"[guard] WARNING: baseline {p} not found — guard references unavailable.")
        return {}
    return json.loads(p.read_text()).get("per_class_acc", {})


BASELINE_GAME7_PER_CLASS = _load_baseline_per_class("game7_results.json")
BASELINE_HELDOUT_PER_CLASS = _load_baseline_per_class("held_out_aggregate.json")
BASELINE_EMPTY_GAME7 = BASELINE_GAME7_PER_CLASS.get("empty", float("nan"))
BASELINE_EMPTY_HELDOUT = BASELINE_HELDOUT_PER_CLASS.get("empty", float("nan"))
print(f"[guard] baseline empty acc — game7={BASELINE_EMPTY_GAME7:.4f}  "
      f"held-out(2/6)={BASELINE_EMPTY_HELDOUT:.4f}  "
      f"(alarm if a run drops > {OVERCORRECTION_EMPTY_TOL:.3f} below)")
print("\033[92m✓ Cell 2b — Over-correction guard references — OK\033[0m")




# %% [Cell 3 — Load manual labels + verify PGN training games]
...
# Stage 3 uses ALL 30 manual labels (no subsetting) PLUS full PGN games 4
# and 5 (training labels combined below in Cell 5).
STAGE3_MANUAL_N = 30
picked_rows = []
...
stage3_manual_df = pd.DataFrame(picked_rows).reset_index(drop=True)
assert len(stage3_manual_df) == STAGE3_MANUAL_N, (
    f"selected {len(stage3_manual_df)} manual frames, expected "
    f"{STAGE3_MANUAL_N} (all manual labels). Check data/real_labels.csv "
    f"row count."
)
games_covered = set(stage3_manual_df["game"].unique())
assert games_covered == {"game8", "game9", "game10", "game11"}, (
    f"all 4 manual-label games must be represented; got {games_covered}"
)

stage3_manual_path = f"{RESULTS_DIR}/stage3_manual_manifest.csv"
stage3_manual_df.to_csv(stage3_manual_path, index=False)
print(f"\nSelected stage-3 MANUAL training manifest (n={len(stage3_manual_df)}):")
print(stage3_manual_df[["game", "ply", "image_name", "fen"]].to_string(index=False))
print(f"\nwrote {stage3_manual_path}")
print(f"per-game manual picks: {stage3_manual_df['game'].value_counts().sort_index().to_dict()}")

# --- Stage 3 ALSO trains on full game4 + game5 PGN data.
# Counts: game4 = 184 frames, game5 = 109 frames (verify after load).
train_pgn_frame_counts = {}
for N in TRAIN_PGN_GAMES:
    gt_csv = f"{PROJECT_ROOT}/data/game{N}_per_frame/gt.csv"
    assert Path(gt_csv).exists(), (
        f"training PGN labels missing: {gt_csv}. game{N} must have a "
        f"gt.csv in data/game{N}_per_frame/."
    )
    df = pd.read_csv(gt_csv)
    train_pgn_frame_counts[N] = len(df)
    print(f"  PGN game{N}: {len(df)} frames will be added to training")

total_real_frames = STAGE3_MANUAL_N + sum(train_pgn_frame_counts.values())
print(f"\nStage 3 total real training frames: "
      f"{STAGE3_MANUAL_N} manual + "
      f"{sum(train_pgn_frame_counts.values())} PGN "
      f"({total_real_frames} total)")

print("\033[92m✓ Cell 3 — Manual + PGN training data verified — OK\033[0m")
...
# Manual-labels portion (30 frames from games 8-11).
manual_train_dataset = ManualLabelsDataset(
    stage3_manual_df, REAL_IMAGES_ROOT, transform=train_transform,
)
print(f"manual_train_dataset (30 frames × 64): "
      f"{len(manual_train_dataset):,} samples")

# NOTE: RealGameDataset is defined in Cell 5. Stage 3 forward-references
# it by deferring train_dataset construction to a small block at the
# top of Cell 5, immediately after RealGameDataset is defined. This
# keeps cell ordering consistent with stage 2.
...
# %% [Cell 5 — RealGameDataset (game7 monitor + PGN training games 4/5 + held-out 2/6 test)]
...
# --- Stage 3: append full game4 + game5 PGN datasets as training data.
pgn_train_datasets = []
for N in TRAIN_PGN_GAMES:
    gt_csv = f"{PROJECT_ROOT}/data/game{N}_per_frame/gt.csv"
    images_dir = f"{PROJECT_ROOT}/data/game{N}_per_frame/images"
    ds = RealGameDataset(
        gt_csv, images_dir, game_name=f"game{N}",
        transform=train_transform,  # SAME augmentation as manual labels
    )
    pgn_train_datasets.append(ds)
    print(f"  PGN train ds for game{N}: {len(ds):,} samples "
          f"({ds.manifest['image_name'].nunique()} frames × 64)")

# Combine manual labels + PGN games into a single training dataset.
from torch.utils.data import ConcatDataset
train_dataset = ConcatDataset([manual_train_dataset] + pgn_train_datasets)
print(f"\ntrain_dataset (manual + PGN concat): {len(train_dataset):,} "
      f"samples total")

print("\033[92m✓ Cell 5 — RealGameDataset (game7) + PGN train concat — OK\033[0m")




# %% [Cell 6 — DataLoaders (NO weighted sampler — ~20,700 samples; class
# imbalance from natural PGN distribution preserved on purpose)]
...
# --- Synthetic rehearsal (Step 3). When REHEARSAL_FRAC == 0 we keep the EXACT
# s00 loader (real-only, shuffle=True) → byte-equivalent. When > 0 we mix v1 synth
# TRAIN squares into each batch via a WeightedRandomSampler (mirrors Stage 5's
# exposure mechanism), drawing synth from dataset_v1 MINUS the 5% monitor slice so
# the forgetting probe stays clean. games 2/6 (test) and game7 (monitor) are real
# images, absent from the synth manifest, so they can never enter rehearsal.
if REHEARSAL_FRAC > 0:
    from torch.utils.data import WeightedRandomSampler
    rehearsal_manifest = synth_manifest[
        ~synth_manifest["source_image"].isin(slice_imgs)   # exclude monitor slice
    ].reset_index(drop=True)
    # Safety asserts (confirmed before launch): no monitor-slice overlap.
    assert set(rehearsal_manifest["source_image"]).isdisjoint(slice_imgs), \
        "rehearsal pool overlaps the 5% synth-monitor slice!"
    synth_rehearsal_dataset = ChessSquareDataset(
        rehearsal_manifest, SYNTH_CORNERS_PATH, dataset_dir=SYNTH_DATASET_DIR,
        transform=train_transform,   # same augmentation as the real side / Stage 5
    )
    n_real = len(train_dataset)
    n_synth_r = len(synth_rehearsal_dataset)
    # ConcatDataset: real first [0, n_real), synth second [n_real, n_real+n_synth_r).
    mixed_train_dataset = ConcatDataset([train_dataset, synth_rehearsal_dataset])
    w_real = (1.0 - REHEARSAL_FRAC) / n_real
    w_synth = REHEARSAL_FRAC / n_synth_r
    sample_weights = torch.tensor(
        [w_real] * n_real + [w_synth] * n_synth_r, dtype=torch.double)
    # Keep REAL exposure ~constant vs s00: ~n_real real draws/epoch.
    num_samples = int(round(n_real / (1.0 - REHEARSAL_FRAC)))
    train_sampler = WeightedRandomSampler(
        weights=sample_weights, num_samples=num_samples, replacement=True)
    train_loader = DataLoader(
        mixed_train_dataset, batch_size=BATCH_SIZE, sampler=train_sampler,
        num_workers=NUM_WORKERS_REAL, pin_memory=True, persistent_workers=True,
        worker_init_fn=_worker_init_fn, drop_last=False,
    )
    print(f"[rehearsal] frac={REHEARSAL_FRAC}: real={n_real:,} + synth_pool={n_synth_r:,} "
          f"(dataset_v1 minus {len(slice_imgs)} monitor imgs); num_samples/epoch={num_samples:,} "
          f"(~{(1-REHEARSAL_FRAC)*BATCH_SIZE:.0f} real + {REHEARSAL_FRAC*BATCH_SIZE:.0f} synth per batch of {BATCH_SIZE})")
else:
    train_loader = DataLoader(
...
# %% [Cell 7b — Class-balanced weights (Option A) — REAL TRAIN ONLY]
# The ONLY behavioral change vs. the frozen stage-3 baseline. Counts come
# exclusively from the real training squares (30 manual + game4 PGN + game5 PGN
# = the ConcatDataset built in Cell 5). Synthetic data is NOT counted; the test
# set (games 2/6) and the monitor (game7) are NEVER touched here.
#
# Formula (single knob s = BALANCE_STRENGTH):
#   base = 1/sqrt(count)            [inv_sqrt, default — gentle]
#        | 1/eff,  eff=(1-b^n)/(1-b) [effective_num, beta=BALANCE_BETA]
#   base_norm = base / mean(base)                          (mean -> 1.0)
#   w = (1-s)*ones + s*base_norm,  then renormalize to mean 1.0
#     s=0.0 -> all ones  -> identical gradient to plain CrossEntropyLoss
#     s=1.0 -> full normalized base
# Normalizing to mean 1.0 keeps the effective learning rate stable across s.

# Count labels from the in-memory training manifests (no image decoding).
_train_label_frames = [manual_train_dataset.manifest["label"]] + [
    ds.manifest["label"] for ds in pgn_train_datasets
]
_train_labels = pd.concat(_train_label_frames, ignore_index=True).to_numpy()
class_counts = np.bincount(_train_labels, minlength=NUM_CLASSES).astype(np.int64)
_n_train_sq = int(class_counts.sum())

_safe_counts = np.maximum(class_counts, 1)
if BALANCE_FORMULA == "inv_sqrt":
    _base = 1.0 / np.sqrt(_safe_counts.astype(np.float64))
elif BALANCE_FORMULA == "effective_num":
    _eff = (1.0 - BALANCE_BETA ** _safe_counts) / (1.0 - BALANCE_BETA)
    _base = 1.0 / _eff
else:
    raise ValueError(f"unknown balance_formula {BALANCE_FORMULA!r}")
_base_norm = _base / _base.mean()
_w = (1.0 - BALANCE_STRENGTH) * np.ones(NUM_CLASSES) + BALANCE_STRENGTH * _base_norm
_w = _w / _w.mean()                       # renormalize -> mean 1.0
class_weights_np = _w.astype(np.float32)
class_weights = torch.tensor(class_weights_np, dtype=torch.float32, device=DEVICE)

print(f"Class weights — formula={BALANCE_FORMULA}  strength={BALANCE_STRENGTH}  "
      f"(real-train squares={_n_train_sq:,})")
print(f"  {'cls':>6} {'count':>7} {'freq':>8} {'weight':>8}")
for _c in range(NUM_CLASSES):
    print(f"  {CLASS_SHORT[_c]:>6} {int(class_counts[_c]):>7} "
          f"{class_counts[_c]/max(_n_train_sq,1):>8.4f} {class_weights_np[_c]:>8.3f}")
assert abs(float(class_weights_np.mean()) - 1.0) < 1e-5, "weight mean must be 1.0"
...
class_weights_payload = {
    ...
    "beta": BALANCE_BETA if BALANCE_FORMULA == "effective_num" else None,
    "n_train_squares": _n_train_sq,
    "class_short": CLASS_SHORT,
    "counts": {CLASS_SHORT[c]: int(class_counts[c]) for c in range(NUM_CLASSES)},
    "weights": {CLASS_SHORT[c]: float(class_weights_np[c]) for c in range(NUM_CLASSES)},
    "note": ("strength=0.0 yields all-ones weights == plain CrossEntropyLoss; "
             "counts from 30 manual + game4 + game5 PGN only (no synth, no test/monitor)."),
}
Path(f"{RESULTS_DIR}/class_weights.json").write_text(
    json.dumps(class_weights_payload, indent=2))
print(f"wrote {RESULTS_DIR}/class_weights.json")
print("\033[92m✓ Cell 7b — Class weights — OK\033[0m")
...
# Training uses the class-balanced weights (Option A). Evaluation uses UNWEIGHTED
# CE so logged val losses stay directly comparable to the frozen baseline — the
# weight only changes the training gradient, never any reported metric.
...
print(f"Loss: weighted CE for training (strength={BALANCE_STRENGTH}), "
      f"unweighted CE for eval logging.")
...
# %% [Cell 8b — L2-SP weight-anchoring (Step 2). Disabled when L2SP_LAMBDA == 0.]
# L2-SP (Li et al. 2018): anchor the BACKBONE to its synth init theta_0 instead of
# letting plain weight-decay pull it toward zero. Penalty added to the TRAINING loss:
#     L2SP_LAMBDA * sum_{p in backbone} || theta_p - theta0_p ||^2
# Applied to conv1/bn1/layer1-4 only (NOT fc — the 13-class head is new, no meaningful
# theta_0). When anchoring is on, weight_decay is turned OFF on the backbone group so the
# two penalties don't double-count; fc keeps its normal weight_decay. theta_0 is snapshotted
# once, right after the v1 weights are (re)loaded in Cell 11, BEFORE Phase A. Phase A freezes
# the backbone, so the penalty only does work in Phase B (epochs 6-30) when it unfreezes.
#
# BYTE-EQUIVALENCE: at L2SP_LAMBDA == 0 this cell installs NOTHING — THETA0 stays empty,
# active_reg_fn stays None, and Cell 11 keeps the original single-group SGD(model.parameters(),
# weight_decay=WEIGHT_DECAY). So lambda 0.0 reproduces s00 exactly (same as balance_strength 0.0).
L2SP_BACKBONE_PREFIXES = ("conv1", "bn1", "layer1", "layer2", "layer3", "layer4")
THETA0 = {}            # name -> detached on-device tensor; filled in Cell 11 if lambda>0
active_reg_fn = None   # set to l2sp_penalty at the Phase A->B transition if lambda>0


def _is_backbone(name):
    return name.startswith(L2SP_BACKBONE_PREFIXES)


def snapshot_theta0(model):
    """Capture backbone weights as the anchor theta_0 (detached, cloned, on device)."""
    snap = {n: p.detach().clone() for n, p in model.named_parameters() if _is_backbone(n)}
    print(f"[L2-SP] snapshotted theta_0 for {len(snap)} backbone tensors "
          f"({sum(t.numel() for t in snap.values()):,} params).")
    return snap


def l2sp_penalty(model):
    """L2SP_LAMBDA * sum over backbone params of ||theta - theta0||^2."""
    sq = 0.0
    for n, p in model.named_parameters():
        if _is_backbone(n) and n in THETA0:
            sq = sq + ((p - THETA0[n]) ** 2).sum()
    return L2SP_LAMBDA * sq


if L2SP_LAMBDA > 0:
    print(f"[L2-SP] ENABLED — lambda={L2SP_LAMBDA}; backbone weight_decay will be set to 0 "
          f"in Phase B, fc keeps wd={WEIGHT_DECAY}. (Active in Phase B only.)")
else:
    print("[L2-SP] disabled (lambda=0) — training path byte-equivalent to s00.")
print("\033[92m✓ Cell 8b — L2-SP definitions — OK\033[0m")
...
def set_bn_eval(model):
    """Put all BatchNorm modules in eval mode: they normalize with the stored
    running stats and do NOT update them. Affine weight/bias stay trainable
    (requires_grad is untouched). Used to freeze the synth-init BN statistics."""
...
def train_one_epoch(model, loader, criterion, optimizer, device, print_every=50,
                    reg_fn=None, freeze_bn=False):
    model.train()
    if freeze_bn:                # keep BN running stats fixed at the synth init
        set_bn_eval(model)
    total_loss = 0.0
    total_correct = 0
    total_count = 0
    ...
    for i, (xb, yb) in enumerate(loader, 1):
        ...
        if reg_fn is not None:           # L2-SP weight-anchoring penalty (Phase B only)
...
# [1b-extra] Confirm PGN-game samples are also augmented (not just manual).
last_idx = len(train_dataset) - 1
s1_pgn, _ = train_dataset[last_idx]
s2_pgn, _ = train_dataset[last_idx]
pgn_diff = float(np.abs(s1_pgn.numpy() - s2_pgn.numpy()).mean())
print(f"  mean |s1 - s2| reading train_dataset[{last_idx}] (PGN sample) "
      f"twice: {pgn_diff:.4f}")
assert pgn_diff > 0.01, (
    f"PGN samples not augmented — mean abs diff {pgn_diff:.4f}. "
    f"Did RealGameDataset receive transform=train_transform?"
)

# [1c] Aug visualization (16 random samples drawn from full concat train_dataset)
...
plt.suptitle(
    f"Stage-3 augmented training crops "
...
# [1c-b] Sample plot of the 30 MANUAL training boards only with labels
# overlaid. PGN frames (~290) are NOT plotted here — they don't need
# per-frame eyeballing and would overwhelm a single figure.
print("\n[1c-b] Stage3 manual-frames sample plot (warped boards + labels):")
fig, axes = plt.subplots(5, 6, figsize=(24, 20))
for ax, (_, row) in zip(axes.ravel(), stage3_manual_df.iterrows()):
    ...
    ax.set_xticks([]); ax.set_yticks([])
plt.suptitle("Stage3 training set — 30 manual labels (PGN frames game4+game5 added but not plotted here)", fontsize=12)
plt.tight_layout(rect=(0, 0, 1, 0.96))
train_samples_path = f"{PLOTS_DIR}/stage3_manual_samples.png"
...
sm_loss_pre, sm_acc_pre, _, _ = evaluate(
    model, synth_monitor_loader, criterion_eval, DEVICE, "synth_monitor")
rv_loss_pre, rv_acc_pre, _, _ = evaluate(
    model, real_val_loader, criterion_eval, DEVICE, "real_val")
...
init_loss = criterion_train(logits, yb)
...
# L2-SP: snapshot theta_0 from the freshly loaded v1 backbone, BEFORE Phase A.
# Only when anchoring is enabled — keeps lambda=0 byte-identical to s00.
if L2SP_LAMBDA > 0:
    THETA0 = snapshot_theta0(model)
...
for epoch in range(1, NUM_EPOCHS + 1):
    ...
    if phase == "B" and not phase_b_started:
        ...
        unfreeze_all(model)
        if L2SP_LAMBDA > 0:
            # L2-SP: anchored backbone group has weight_decay=0 (the L2-SP penalty
            # replaces decay-toward-zero with decay-toward-theta_0); fc keeps wd.
            bb_params = [p for n, p in model.named_parameters() if _is_backbone(n)]
            fc_params = [p for n, p in model.named_parameters() if not _is_backbone(n)]
            optimizer = torch.optim.SGD(
                [{"params": bb_params, "weight_decay": 0.0},
                 {"params": fc_params, "weight_decay": WEIGHT_DECAY}],
                lr=LR_FULL_FT, momentum=MOMENTUM,
            )
            active_reg_fn = l2sp_penalty
            print(f"[phase A→B] all params unfrozen; L2-SP ON (lambda={L2SP_LAMBDA}); "
                  f"SGD(lr={LR_FULL_FT}, momentum={MOMENTUM}; backbone wd=0, fc wd={WEIGHT_DECAY}); "
                  f"no scheduler")
        else:
            # Byte-equivalent to s00: original single-group optimizer, no penalty.
            optimizer = torch.optim.SGD(
                model.parameters(),
    ...
    train_loss, train_acc = train_one_epoch(
        model, train_loader, criterion_train, optimizer, DEVICE, print_every=100,
        reg_fn=active_reg_fn, freeze_bn=FREEZE_BN)
    if active_reg_fn is not None:
        with torch.no_grad():
            print(f"    [L2-SP] end-of-epoch penalty = {float(l2sp_penalty(model)):.4f}  "
                  f"(lambda={L2SP_LAMBDA})")
    ...
    # --- Over-correction trap watch: empty + pawns must not collapse. ---
    empty_acc_ep = rv_per_class[12]; wp_acc_ep = rv_per_class[0]; bp_acc_ep = rv_per_class[6]
    _empty_flag = (not math.isnan(empty_acc_ep) and not math.isnan(BASELINE_EMPTY_GAME7)
                   and empty_acc_ep < BASELINE_EMPTY_GAME7 - OVERCORRECTION_EMPTY_TOL)
    _alarm = "  \033[91m← EMPTY DROPPED — possible over-correction\033[0m" if _empty_flag else ""
    print(f"    [trap watch] real_val empty={_fmt(empty_acc_ep)} "
          f"(baseline {BASELINE_EMPTY_GAME7:.3f})  wP={_fmt(wp_acc_ep)}  "
          f"bP={_fmt(bp_acc_ep)}{_alarm}")
...
# %% [Cell 17 — Held-out games: dataset + loader for games 2/6 (stage 3 partition)]
real_test_datasets = {}
real_test_loaders = {}
...
for N, loader in real_test_loaders.items():
    ...
    g_loss, g_acc, g_preds, g_labels = evaluate(
        model, loader, criterion_eval, DEVICE, f"game{N}")
...
# %% [Cell 19 — Aggregate across held-out games (games 2/6 — stage 3 partition)]
...
plot_confusion_matrix(
    cm_c,
    f"Held-out aggregate (games 2/6 — stage 3 partition) — per-square acc={combined_per_square_acc:.4f}",
...
# %% [Cell 19b — Over-correction guard verdict (the trap)]
# Compare the weighted run's `empty` (and pawn) accuracy on game7 + games 2/6
# against the FROZEN baseline. A meaningful drop in `empty` means we traded the
# easiest/most-common class for rare-piece recall — the failure mode to avoid.
def _drop(after, before):
    if math.isnan(after) or math.isnan(before):
        return float("nan")
    return before - after  # positive = regression


g7_empty_after = g7_per_class[12]
ho_empty_after = combined_per_class[12]
g7_empty_drop = _drop(g7_empty_after, BASELINE_EMPTY_GAME7)
ho_empty_drop = _drop(ho_empty_after, BASELINE_EMPTY_HELDOUT)
over_corrected = (
    (not math.isnan(g7_empty_drop) and g7_empty_drop > OVERCORRECTION_EMPTY_TOL)
    or (not math.isnan(ho_empty_drop) and ho_empty_drop > OVERCORRECTION_EMPTY_TOL)
)

guard_payload = {
    "balance_strength": BALANCE_STRENGTH,
    "balance_formula": BALANCE_FORMULA,
    "empty_tol": OVERCORRECTION_EMPTY_TOL,
    "game7": {
        "empty_baseline": BASELINE_EMPTY_GAME7, "empty_after": g7_empty_after,
        "empty_drop": g7_empty_drop,
        "wP_after": g7_per_class[0], "bP_after": g7_per_class[6],
    },
    "held_out_2_6": {
        "empty_baseline": BASELINE_EMPTY_HELDOUT, "empty_after": ho_empty_after,
        "empty_drop": ho_empty_drop,
        "wP_after": combined_per_class[0], "bP_after": combined_per_class[6],
    },
    "over_correction_flag": bool(over_corrected),
}
Path(f"{RESULTS_DIR}/over_correction_guard.json").write_text(
    json.dumps(guard_payload, indent=2))

print("\n[over-correction guard]")
print(f"  game7   empty: baseline {BASELINE_EMPTY_GAME7:.4f} -> after "
      f"{g7_empty_after:.4f}  (drop {g7_empty_drop:+.4f})")
print(f"  2/6     empty: baseline {BASELINE_EMPTY_HELDOUT:.4f} -> after "
      f"{ho_empty_after:.4f}  (drop {ho_empty_drop:+.4f})")
if over_corrected:
    print("\033[91m" + "!" * 64)
    print(f"!!! OVER-CORRECTION WARNING (strength={BALANCE_STRENGTH}): `empty` "
          f"accuracy dropped > {OVERCORRECTION_EMPTY_TOL:.3f} below baseline.")
    print("!!! Rare-piece weighting is too aggressive — empty squares are being "
          "hallucinated as pieces. Prefer a lower --balance_strength.")
    print("!" * 64 + "\033[0m")
else:
    print(f"\033[92m  OK — `empty` held within {OVERCORRECTION_EMPTY_TOL:.3f} of "
          f"baseline (no over-correction).\033[0m")
print(f"wrote {RESULTS_DIR}/over_correction_guard.json")
print("\033[92m✓ Cell 19b — Over-correction guard verdict — OK\033[0m")
...
# NOTE on caveat: v1 baseline held-out was on games 2/4/5/6; stage 3 is on
# games 2/6. The direct comparison is not perfectly clean, but is still
# informative since games 2 and 6 are a strict subset of v1's test set.
if v1_baseline_held_out and "per_square_acc" in v1_baseline_held_out:
    ...
    verdict = (
        f"**Verdict:** stage3_323 {'BEAT' if beat else 'did NOT beat'} v1 zero-shot "
        f"on held-out — "
        f"{combined_per_square_acc:.4f} (games 2/6) vs. {v1_held_out:.4f} "
        f"(v1 on games 2/4/5/6) "
        f"(Δ = {combined_per_square_acc - v1_held_out:+.4f}; partition differs, "
        f"interpret as directional)."
    )
else:
    verdict = (
        f"**Verdict:** stage3_323 per-square acc on held-out (games 2/6) is "
...
# delta block). Reads stage 2 results from disk if present; falls back to
# "n/a" rows otherwise. NOTE: stage 2's held-out was games 2/4/5/6 while
# stage 3 is games 2/6 — comparison is NOT directly valid. The matched-
# partition bridge in Cell 22 (stage 2 re-evaluated on games 2/6) is the
# correct cross-stage table.
# Variable name `stage1_compare_lines` / `s1_paths` is kept from the
# stage 2 template lineage; the *contents* now refer to stage 2 paths.
stage1_compare_lines = [
    "## Comparison to stage2_30 (different test partition — see re-eval below)",
    "",
    "⚠️ Stage 2 numbers below are on games 2/4/5/6 (full held-out). "
    "Stage 3 numbers are on games 2/6 only. NOT directly comparable. "
    "See 'Stage 2 reevaluated on games 2/6' section for the "
    "matched-partition bridge.",
    "",
...
summary_lines = [
    ...
    "(NOT stage 1 or stage 2 weights).",
    "- **Training data:** 30 manual-label frames (games 8-11) + "
    "full game4 PGN (184 frames) + full game5 PGN (109 frames). "
    "~323 total real frames, ~20,700 squares.",
    ...
    f"lr={LR_FULL_FT}; no scheduler.",
    f"- **Aug:** color jitter @{COLOR_JITTER_APPLY_PROB} → shear @{SHEAR_APPLY_PROB} "
    f"(±8°) → noise @{NOISE_APPLY_PROB} (std={NOISE_STD}), applied to BOTH manual "
    f"and PGN samples.",
    f"- **Sampler:** shuffle=True (NO weighted sampler; natural PGN class "
    f"distribution preserved on purpose).",
...
# %% [Cell 22 — Stage 2 re-eval on games 2/6 (cross-partition bridge)]
print("=" * 64)
print("Loading stage_2 best_real checkpoint and re-evaluating on the")
print("stage 3 test partition (games 2/6) for direct comparison.")
print("=" * 64)
...
if not Path(stage2_ckpt_path).exists():
    ...
    print(f"[Cell 22] skipping re-eval. Stage 3 summary.md will not have"
          f" the bridge row.")
else:
    ...
    # Re-evaluate on the SAME loaders we just built (games 2/6).
    s2_per_game_acc = {}
    s2_combined_preds = []
    s2_combined_labels = []
    ...
    for N, loader in real_test_loaders.items():
        _, g_acc, g_preds, g_labels = evaluate(
            stage2_model, loader, criterion_eval, DEVICE, f"stage2_on_game{N}")
    ...
    # Per-board accuracy for stage 2 on games 2/6.
    s2_per_board = (
    ...
    Path(f"{RESULTS_DIR}/stage2_reeval_on_games_2_6.json").write_text(
        json.dumps(stage2_reeval, indent=2))
    print(f"\nstage 2 on games 2/6 partition:")
    ...
    print(f"wrote {RESULTS_DIR}/stage2_reeval_on_games_2_6.json")

    # Append to summary.md: a new "matched-partition bridge" section.
    bridge_lines = [
        "",
        ...
        "stage 3 (games 2/6 only). This is the cross-stage comparison "
        "to use, NOT the table above.",
        "",
    ...
    for cls in range(NUM_CLASSES):
        s2 = s2_per_class[cls]
        s3 = combined_per_class[cls]
        if s2 == s2 and s3 == s3:
            bridge_lines.append(
                f"| {CLASS_SHORT[cls]} | {s2:.4f} | {s3:.4f} | "
                f"{s3 - s2:+.4f} |"
            )
        else:
            bridge_lines.append(
                ...
                f"{s3 if s3 == s3 else float('nan'):.4f} | n/a |"
            )
    bridge_lines.append("")
