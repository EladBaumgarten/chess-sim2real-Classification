# Run — `dino_stage5_5` (DINOv2 ViT-S/14, combined, NEW SPLIT) + mini-experiment verdict

> ⚠️ **NOT comparable to the original dino_stage3/stage5 numbers.** Different test set (**game7
...
## New split (only change vs original dino_stage5)
...
- Flags on `--mode stage5`: `--train_pgn_games 4,5,2 --val_game game6 --test_games 7`. Everything
  else identical to dino_stage5 (DINOv2 ViT-S/14, 224/256 tokens, AdamW wd0.05 cosine, batch64 seed42,
...
**WINNER: `dino_stage5_5` (combined) — by +0.020 piece-only on game7.** Combined beats sequential
...
{sequential FT, combined} for DINOv2 ViT-S/14 is **combined (`dino_stage5_5`)**, by **+0.020
...
`checkpoints/dino_stage5_5/{best_real.pt(ep16), best_synth_monitor.pt, latest.pt}`;
`results/dino_stage5_5/{training_log.csv, recipe.json (split metadata), game6_results.json,
heldout_game7_eval.json, synth_monitor_results.json, predictions/*.npy}`; `plots/dino_stage5_5/*.png`.
