# Ablation — `LabelSmoothing`: add label smoothing to `dino_combindedGame6`'s loss
...
`dino_combindedGame6` uses the bare loss: plain `nn.CrossEntropyLoss()`. The session's confidence
...
1. **Label smoothing is the wrong loss element for this model** — net negative on accuracy and on
   aggregate calibration. `dino_combindedGame6` (plain CE) remains the better model.
...
- `dino/ablation_LabelSmoothing/checkpoints/{best_real.pt (ep15, game2-selected), best_synth_monitor.pt, latest.pt}` (no `best_game7_diag.pt` — per-epoch game7 diag was off)
- `dino/ablation_LabelSmoothing/results/{labelsmooth_vs_combined_compare.json, recipe.json, heldout_game7_eval.json, game2_results.json, synth_monitor_results.json, training_log.csv, predictions/*.npy}`
- `dino/ablation_LabelSmoothing/plots/{training_curves.png, game7_cm.png, game2_cm.png, heldout_game7_cm.png, synth_monitor_cm.png}`
...
- Calibration builder: `dino/ablation_LabelSmoothing/make_calib_compare.py`
