# Run 2 — `dino_stage3` (DINOv2 ViT-S/14, sequential fine-tune synth→real)
...
- **Selected epoch 7** → `checkpoints/dino_stage3/best_real.pt`. Early-stopped at epoch 13
...
`checkpoints/dino_stage3/{best_real.pt(ep7), best_synth_monitor.pt, latest.pt}`;
`results/dino_stage3/{training_log.csv, recipe.json, games_2_6_eval.json, game7_results.json,
synth_monitor_results.json, predictions/*.npy}`; `plots/dino_stage3/*.png`.

## Next

`dino_stage5` — combined synth+real from DINOv2-pretrained, WeightedRandomSampler 50/50
