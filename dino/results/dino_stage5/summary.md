# Run 3 — `dino_stage5` (DINOv2 ViT-S/14, combined synth+real from pretrained)
...
- **Selected epoch 19** → `checkpoints/dino_stage5/best_real.pt`. Ran the full 20 epochs (kept
...
`checkpoints/dino_stage5/{best_real.pt(ep19), best_synth_monitor.pt, latest.pt}`;
`results/dino_stage5/{training_log.csv, recipe.json, games_2_6_eval.json, game7_results.json,
synth_monitor_results.json, predictions/*.npy}`; `plots/dino_stage5/*.png`.

## Next

`dino_linprobe` — backbone FROZEN all epochs, head only, on combined synth+real 50/50 (same data
