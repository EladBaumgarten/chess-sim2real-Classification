# Run — `dino_stage3_5` (DINOv2 ViT-S/14, sequential FT, NEW SPLIT)

> ⚠️ **NOT comparable to the original dino_stage3/stage5 numbers.** This run uses a DIFFERENT
...
## New split (the only change vs the original dino_stage3)
...
`--train_pgn_games 4,5,2 --val_game game6 --test_games 7`. Everything else identical to dino_stage3:
...
LayerNorm low-forgetting profile (orig dino_stage3 was −0.0214).

## Artifacts

`checkpoints/dino_stage3_5/{best_real.pt(ep6), best_synth_monitor.pt, latest.pt}`;
`results/dino_stage3_5/{training_log.csv, recipe.json (split metadata), game6_results.json,
heldout_game7_eval.json, synth_monitor_results.json, predictions/*.npy}`; `plots/dino_stage3_5/*.png`.

## Next

`dino_stage5_5` — same new split, combined synth+real (+game2) from DINOv2-pretrained, weighted
