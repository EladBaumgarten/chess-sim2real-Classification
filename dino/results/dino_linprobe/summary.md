# Run 4 — `dino_linprobe` (DINOv2 ViT-S/14, FROZEN backbone, head-only)

Linear probe: backbone **frozen for all epochs** (no Phase B), train only the 13-class head, on
the SAME combined synth+real 50/50 data as `dino_stage5`. Identical to stage5 in every way EXCEPT
...
`dino_linprobe` keeps the backbone at raw DINOv2-pretrained (never chess-adapted) and only fits a
...
the win is backbone adaptation. This makes `dino_stage5` (combined, FT) the headline DINO result.

## Artifacts

`checkpoints/dino_linprobe/{best_real.pt(ep18), best_synth_monitor.pt, latest.pt}`;
`results/dino_linprobe/{training_log.csv, recipe.json, games_2_6_eval.json, game7_results.json,
synth_monitor_results.json, predictions/*.npy}`; `plots/dino_linprobe/*.png`.
