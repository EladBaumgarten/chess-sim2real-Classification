"""
Project 2 — Fine-tuning STAGE 1 (10 real images).

# FINE-TUNING (NOT ZERO-SHOT). 10 real images from games 8-11 are in
# training. game7 is the monitor and gates checkpoint selection.
# Pretrained weights: zero_shot/results/best_synth.pt (v1 baseline,
# peak game7 real_val 0.5923) — NOT zero_shot_v1.5.

Two-phase recipe (Wölflein 2021, §4.3):
  Phase A (epochs 1-5)   — freeze backbone, train fc only @ lr=1e-3.
  Phase B (epochs 6-30)  — unfreeze all, lr=1e-4 with StepLR(step_size=15,
                            gamma=0.1) → drops to 1e-5 at absolute epoch 21.
...
sampling — 640 samples is too small). Checkpoint selection: real_val_acc
on game7. Early stop patience=8.

Stage 1 / 3. The other two stages share an IDENTICAL test partition:
...
# === Synth data (v1, NOT v1.5) for catastrophic-forgetting probe ===
...
# === Experiment dirs ===
EXP_DIR = f"{PROJECT_ROOT}/fine_tuning/stage1_10"
...
# %% [Cell 3 — Load manual labels + stratified pick of 10 training frames]
...
# Stratified picker: target 10 total, 4 games. Allocate 3/3/2/2 by default
# (matches even distribution); if a game has fewer than its share, take all
# available and redistribute the remainder. SEED=42 controls within-game
# sampling order so picks are deterministic.
ALLOC = {"game8": 3, "game9": 3, "game10": 2, "game11": 2}
STAGE1_N = 10

rng = random.Random(SEED)
picked_rows = []
total_alloc = sum(ALLOC.values())
assert total_alloc == STAGE1_N, f"allocation must sum to {STAGE1_N}, got {total_alloc}"

for game, k in ALLOC.items():
    sub = manual_df[manual_df["game"] == game].sort_values("ply").reset_index(drop=True)
    available = len(sub)
    assert available >= k, (
        f"{game} has only {available} manual labels but allocation asks for {k}. "
        f"Re-tune ALLOC to match the observed per-game distribution."
    )
    indices = list(range(available))
    rng.shuffle(indices)
    picked = sub.iloc[sorted(indices[:k])]   # sort by index for reproducible printing
    for _, row in picked.iterrows():
        picked_rows.append({
            "game_num": int(game.replace("game", "")),
            "game": game,
            "image_name": row["image_path"],   # relative to data/, e.g. c06/game8/images/frame_*.jpg
            "fen": row["fen"],
            "view": row["view"],
            "label_source": "manual",
            "ply": int(row["ply"]),
...
stage1_train_df = pd.DataFrame(picked_rows).reset_index(drop=True)

assert len(stage1_train_df) == STAGE1_N, \
    f"selected {len(stage1_train_df)} images, expected {STAGE1_N}"
games_covered = set(stage1_train_df["game"].unique())
assert games_covered == {"game8", "game9", "game10", "game11"}, (
    f"all 4 games must be represented; got {games_covered}"
)

stage1_train_path = f"{RESULTS_DIR}/stage1_train_manifest.csv"
stage1_train_df.to_csv(stage1_train_path, index=False)
print(f"\nSelected stage-1 training manifest (n={len(stage1_train_df)}):")
print(stage1_train_df[["game", "ply", "image_name", "fen"]].to_string(index=False))
print(f"\nwrote {stage1_train_path}")
print(f"per-game picks: {stage1_train_df['game'].value_counts().sort_index().to_dict()}")

# Remaining 20 manual labels — reserved for stage 2. Not used here.
reserved_df = manual_df[
    ~manual_df["image_path"].isin(stage1_train_df["image_name"].values)
].copy()
print(f"\nReserved for stage 2: {len(reserved_df)} manual labels. (not used in this run)")

print("\033[92m✓ Cell 3 — Stratified pick of 10 training frames — OK\033[0m")
...
train_dataset = ManualLabelsDataset(
    stage1_train_df, REAL_IMAGES_ROOT, transform=train_transform,
)
print(f"train_dataset (10 frames × 64): {len(train_dataset):,} samples "
      f"({stage1_train_df['game'].nunique()} games × ~2.5 frames/game)")
...
# %% [Cell 6 — DataLoaders (NO weighted sampler — 640 samples is too small)]
...
# [1c] Aug visualization on the 10 chosen training frames (4×4 grid)
print("\n[1c] Augmentation visualization (10 chosen frames → aug_smoke_check.png):")
np.random.seed(SEED + 1)
# Pick one square per frame randomly, then 6 extra random squares (16 total).
frame_indices = []
for img_name in stage1_train_df["image_name"].values:
    sub_mask = train_dataset.manifest["image_name"] == img_name
    sub_idxs = np.where(sub_mask.values)[0]
    frame_indices.append(int(np.random.choice(sub_idxs)))
extra = np.random.choice(len(train_dataset), size=16 - len(frame_indices), replace=False)
viz_indices = frame_indices + list(extra)
...
plt.suptitle(
    f"Stage-1 augmented training crops "
...
# [1c-b] Sample plot of the 10 chosen training boards with labels overlaid.
# This is the "user can sanity-check before training" plot.
print("\n[1c-b] Stage1 train-sample plot (warped boards + labels):")
fig, axes = plt.subplots(2, 5, figsize=(22, 9))
for ax, (_, row) in zip(axes.ravel(), stage1_train_df.iterrows()):
    ...
    ax.set_xticks([]); ax.set_yticks([])
plt.suptitle("Stage1 training set — 10 chosen frames with FEN labels overlaid", fontsize=12)
plt.tight_layout(rect=(0, 0, 1, 0.96))
train_samples_path = f"{PLOTS_DIR}/stage1_train_samples.png"
...
for epoch in range(1, NUM_EPOCHS + 1):
    ...
    if phase == "B" and not phase_b_started:
        ...
        print(f"[phase A→B] all params unfrozen; "
              ...
              f"StepLR(step={LR_STEP_SIZE}, gamma={LR_GAMMA})")
...
if v1_baseline_held_out and "per_square_acc" in v1_baseline_held_out:
    ...
    verdict = (
        f"**Verdict:** stage1_10 {'BEAT' if beat else 'did NOT beat'} v1 zero-shot "
    ...
    verdict = (
        f"**Verdict:** stage1_10 per-square acc on held-out (games 2/4/5/6) is "
...
summary_lines = [
    ...
    f"lr={LR_FULL_FT}; StepLR(step={LR_STEP_SIZE}, gamma={LR_GAMMA}).",
    f"- **Aug:** color jitter @{COLOR_JITTER_APPLY_PROB} → shear @{SHEAR_APPLY_PROB} "
    f"(±8°) → noise @{NOISE_APPLY_PROB} (std={NOISE_STD}).",
    f"- **Sampler:** shuffle=True (NO weighted sampler, 640 samples is too small).",
