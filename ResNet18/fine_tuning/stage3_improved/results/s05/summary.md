...
## Training
- Ran **28** epochs in **56.8 min**.
- Stop reason: `early_stop_patience_8_no_improve_real_val (best=0.9409 at epoch 20)`.
- Best real_val_acc (game7):  **0.9409** at epoch 20.
- Best synth_monitor (5% v1): **0.8850** at epoch 1.

## Catastrophic-forgetting probe (5% slice of dataset_v1)
- Synth-monitor acc BEFORE FT (loaded baseline): **0.9997**
- Synth-monitor acc AFTER FT  (best_real ckpt):  **0.8597**
- **Catastrophic-forgetting Δ: -0.1400**

## Game7 monitor (NOT held-out — used for checkpoint selection)
- Per-square at best_real:  **0.9409**  (before FT: 0.5670; v1 ckpt-epoch real_val: 0.5670; v1 peak real_val: 0.5923)
- Improvement over loaded baseline: **+0.3739**
- Per-board acc: 0/55 = 0.0000
- Mean squares correct/board: 60.22/64
...
|------|---------:|----------:|-----------:|--------------:|---------------:|-------------:|
| game2 | 77 | 4928 | 0.9517 | 0.0260 | 0.8674 | 60.91/64 |
| game6 | 92 | 5888 | 0.8662 | 0.0000 | 0.6541 | 55.43/64 |
| **agg** | 169 | 10816 | **0.9051** | **0.0118** | **0.7471** | 57.93/64 |

**Verdict:** stage3_323 per-square acc on held-out (games 2/6) is **0.9051**. v1 zero-shot held-out aggregate not available on disk — compare via `zero_shot/results/games_2_4_5_6_eval/` if present, or re-evaluate the baseline checkpoint on this exact partition.
...
|--------|--------------------:|-----------------:|--------------:|
| game7 real_val_acc | 0.8037 | 0.9409 | +0.1372 |
| held-out per-sq | 0.8582 | 0.9051 | +0.0469 |
| held-out piece-only | 0.5408 | 0.7471 | +0.2063 |
| forgetting Δ on 5% v1 | -0.0796 | -0.1400 | -0.0605 |

### Per-class on aggregate held-out (different test partition — caveat applies)

| class | stage2_30 (2/4/5/6) | stage3_323 (2/6) | Δ |
|-------|--------------------:|-----------------:|---:|
| wP | 0.8223 | 0.9299 | +0.1076 |
| wR | 0.5578 | 0.9762 | +0.4184 |
| wN | 0.0201 | 0.4142 | +0.3941 |
| wB | 0.5918 | 0.3276 | -0.2642 |
| wQ | 0.1797 | 0.2273 | +0.0475 |
| wK | 0.0866 | 0.5207 | +0.4341 |
| bP | 0.7163 | 0.9624 | +0.2461 |
| bR | 0.4729 | 0.8538 | +0.3809 |
| bN | 0.3066 | 0.2807 | -0.0259 |
| bB | 0.1025 | 0.3143 | +0.2118 |
| bQ | 0.2744 | 0.2214 | -0.0530 |
| bK | 0.1147 | 0.3491 | +0.2344 |
| empty | 0.9889 | 0.9982 | +0.0094 |
...
## Artifacts
- `checkpoints/best_real.pt` (epoch 20, real_val_acc=0.9409) — headline ckpt
- `checkpoints/best_synth_monitor.pt` (epoch 1, synth_monitor_acc=0.8850) — monitor-only
...
|--------|----------------:|------------------:|---:|
| per-sq acc | 0.8578 | 0.9051 | +0.0473 |
| piece-only | 0.6317 | 0.7471 | +0.1155 |
| per-board  | 0.0000 (0/169) | 0.0118 (2/169) | +0.0118 |

### Per-class on games 2/6 (matched partition)

| class | stage2_30 ON 2/6 | stage3_323 ON 2/6 | Δ |
|-------|-----------------:|------------------:|---:|
| wP | 0.8684 | 0.9299 | +0.0615 |
| wR | 0.8143 | 0.9762 | +0.1619 |
| wN | 0.0237 | 0.4142 | +0.3905 |
| wB | 0.5172 | 0.3276 | -0.1897 |
| wQ | 0.1591 | 0.2273 | +0.0682 |
| wK | 0.1716 | 0.5207 | +0.3491 |
| bP | 0.8530 | 0.9624 | +0.1094 |
| bR | 0.5189 | 0.8538 | +0.3349 |
| bN | 0.4211 | 0.2807 | -0.1404 |
| bB | 0.0686 | 0.3143 | +0.2457 |
| bQ | 0.2366 | 0.2214 | -0.0153 |
| bK | 0.0888 | 0.3491 | +0.2604 |
| empty | 0.9910 | 0.9982 | +0.0072 |
