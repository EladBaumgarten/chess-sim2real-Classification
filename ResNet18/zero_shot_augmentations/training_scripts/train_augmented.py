"""
Project 2 — Wölflein-style narrower augmentation (Step 6d ablation,
sibling of zero_shot/train_baseline.py).

NOTE: this file lives in zero_shot_augmentations/ for historical reasons (it
replaced the previous 7-stage 'augmented' variant in-place). The augmentation
pipeline below is the narrower Wölflein-style recipe, NOT the prior 7-stage
stack.

Same architecture (ResNet18 ImageNet-pretrained, 13-class head), same data
(dataset_v1 + game{2,4,5,6,7}), same optimizer (SGD+StepLR), same FEN-disjoint
train/val/test split, same sqrt-inverse-frequency sampler, same 10-epoch
budget with LR step at epoch 4, same checkpoint rule (best by synth_val_acc
only — real_val is monitor-only).  Only change vs. the prior 7-stage variant:

  • Narrower augmentation pipeline (3 stages: color jitter incl. hue + affine/
    shear + mild Gaussian noise).  Removed: random grayscale, Gaussian blur,
    sharpness adjust, random erase.

Hypothesis being tested:
  The prior 7-stage stack appeared too aggressive — train_acc capped at ~99.2%
  by epoch 5, suggesting the augmentation pipeline was degrading the training
  signal more than helping generalization.  A narrower set of only the
  augmentations Wölflein 2021 explicitly used should let the model fit synth
  more cleanly while still gaining the geometric robustness (shear → camera-
  angle invariance) Wölflein identified as the single biggest sim-to-real
  lever (their result: shear took transfer accuracy from 44% → 89%).

IMPORTANT — this is still a zero-shot run:
  - No real images enter training or validation.
  - game7 remains per-epoch real-image MONITOR only — logged but never used
    to gate checkpoint selection.
  - Games 2, 4, 5, 6 remain held-out real TEST only — evaluated once at the
    end with the locked-in best-synth-val checkpoint.
  - We are NOT doing Wölflein's 2-photo fine-tuning protocol — only borrowing
    their augmentation recipe.  Fine-tuning is a future Step 8.

All outputs go to {RESULTS_DIR} = zero_shot_augmentations/results/ and
{PLOTS_DIR} = zero_shot_augmentations/plots/.  Nothing under zero_shot/.

Hard rules unchanged from baseline:
...
BATCH_SIZE = 64
NUM_EPOCHS = 10                # was 20 in baseline — baseline showed synth saturates by epoch 2-3
NUM_WORKERS = 6   # matches system's suggested max; silences the torch warning
LR = 0.001
MOMENTUM = 0.9
WEIGHT_DECAY = 1e-4
LR_STEP_SIZE = 4               # was 7 in baseline — proportional to NUM_EPOCHS=10
LR_GAMMA = 0.1
TRAIN_FRAC, VAL_FRAC, TEST_FRAC = 0.70, 0.15, 0.15

# === Augmentation strengths (WÖLFLEIN-style, narrower than prior 7-stage stack)
# Color augmentation — slightly lower than the prior 7-stage variant; no grayscale.
COLOR_JITTER_BRIGHTNESS = 0.3
COLOR_JITTER_CONTRAST = 0.3
COLOR_JITTER_SATURATION = 0.3
COLOR_JITTER_HUE = 0.05              # was 0.08 in prior variant

# Noise — back to baseline-mild.
GAUSSIAN_NOISE_STD = 0.02            # was 0.05 in prior variant

# Geometric — Wölflein's #1 finding, pushed to highest probability.
SHEAR_PROB = 0.80                    # was 0.50 in prior variant
SHEAR_DEG_RANGE = (-8, 8)            # was (-10, 10) — slightly tighter
AFFINE_TRANSLATE_RANGE = (-0.03, 0.03)   # was (-0.05, 0.05)
AFFINE_SCALE_RANGE = (0.97, 1.03)    # was (0.95, 1.05)

# Apply-probabilities for the two pipeline stages with helpers.
COLOR_JITTER_APPLY_PROB = 0.60       # was 0.80 in prior variant
NOISE_APPLY_PROB = 0.30              # was 0.60 in prior variant

# REMOVED from the prior 7-stage variant (no longer used in this narrower recipe):
#   RANDOM_GRAYSCALE_PROB, GAUSSIAN_BLUR_PROB, GAUSSIAN_BLUR_KERNEL,
#   GAUSSIAN_BLUR_SIGMA_RANGE, SHARPNESS_PROB, SHARPNESS_FACTOR_RANGE,
#   RANDOM_ERASE_PROB, RANDOM_ERASE_AREA_RANGE, RANDOM_ERASE_RATIO_RANGE
# ==============================================================================
...
ZERO_SHOT_DIR = f"{PROJECT_ROOT}/zero_shot_augmentations"   # NEW root, not zero_shot/
...
# Wölflein-style augmentation helpers — only color jitter (incl. hue),
# affine/shear, and Gaussian noise.  Random grayscale, Gaussian blur, sharpness
# adjust, and random erase are intentionally absent — see top-of-file
# docstring for the hypothesis.

def gaussian_noise(x_rgb_uint8):
    """Add Gaussian noise. Strength: GAUSSIAN_NOISE_STD (in [0,1] units)."""
...
def color_jitter(x_rgb_uint8):
    ...
    Strengths: COLOR_JITTER_{BRIGHTNESS,CONTRAST,SATURATION,HUE}."""
    x = x_rgb_uint8.astype(np.float32)
    ...
    # Hue: rotate in HSV space (cv2 hue is uint8 in [0,180])
...
def affine_shear(x_rgb_uint8):
    """Random affine: independent x/y shear + translate + scale. cv2.warpAffine
    with REPLICATE border so edge pixels don't introduce black bars (which the
    model could overfit to as a synthetic-only feature)."""
    ...
    # Compose: shear in radians, then scale, then translate. Build the matrix
    # directly so we control all three in one warp.
    sx = np.tan(np.deg2rad(shear_x_deg))
    sy = np.tan(np.deg2rad(shear_y_deg))
    cx, cy = w / 2.0, h / 2.0
    # Center-anchored shear + scale, then translate
    M = np.array([
...
def train_transform(crop_rgb_uint8):
    ...
    Wölflein 2021 found shear was the single most impactful augmentation for
    sim-to-real on chessboards. Color jitter (incl. hue), scale, and translate
    were the supporting augmentations. We do NOT add grayscale, blur, sharpness,
    or random erase — those are not in Wölflein's recipe and the prior 7-stage
    variant showed they degrade training without sufficient real-image benefit.

    HWC uint8 RGB → HWC uint8 RGB. Tensorization + ImageNet normalize happens
    AFTER this in the model loop (same contract as baseline).

    Order (each gated by its own probability):
      (a) color jitter — 60% — brightness/contrast/saturation/hue jitter
      (b) affine + shear — 80% — Wölflein's key sim-to-real lever
      (c) gaussian noise — 30% — mild sensor noise
    """
    x = crop_rgb_uint8
...
print(f"train_dataset:      {len(train_dataset):>8,d} samples (Wölflein-style augmentation)")
...
# sqrt-inverse-freq: softer than inverse-freq to avoid baking in a strong rare-class prior;
# mitigates the empty→queen/knight hallucination observed in baseline.
...
# 1b. Augmentation actually fires: same sample read twice from the train dataset
#     should produce materially different pixel values because augmentations are
#     stochastic per __getitem__. Pulling two fresh batches from the loader
#     wouldn't test this (different batches contain different samples regardless
#     of augmentation) — we have to read the same index twice.
print("\n[1b] Augmentation-firing check (same sample, two reads):")
idx = 0  # any fixed index in train_dataset
s1, _ = train_dataset[idx]
s2, _ = train_dataset[idx]
# s1, s2 are HWC uint8 numpy arrays before tensorization OR (3,100,100) float32 tensors
# depending on ChessSquareDataset's __getitem__ contract. Handle both.
if hasattr(s1, "numpy"):
    arr1 = s1.float().numpy()
    arr2 = s2.float().numpy()
else:
...
# 4b. Loss sanity check on 3 batches of synth_val: random init should give loss
#     near ln(NUM_CLASSES). One SGD step doesn't move it much, so we expect the
#     measured loss to sit in a wide band around that value.
print("\n[4b] Eval on 3 batches of synth_val (loss sanity check):")
model.eval()
...
#    This is the most important smoke check: a quiet bug in the corner-detection
#    or warp/crop pipeline only surfaces here, and finding it now beats finding
#    it 17 minutes into epoch 1.
print("\n[5] Eval on 3 batches of real_val (Game7Dataset path):")
model.eval()
correct = 0; total = 0
with torch.no_grad():
...
# ZERO-SHOT: no real images enter training or validation. Real evaluation
# uses game7 as monitor only and games 2/4/5/6 as held-out test only.
# Checkpoints are selected by synth_val_acc only. real_val_acc is logged
# but never gates checkpoint saves. This is the same protocol as baseline
# and the prior 7-stage augmented variant; the only experimental variable
# here is augmentation strength.
training_log = []
