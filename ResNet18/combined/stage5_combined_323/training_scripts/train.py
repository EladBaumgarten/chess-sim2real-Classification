"""
Project 2 — Stage 5: FROM-SCRATCH COMBINED training (ImageNet → synth + 353 real).

# NOT FINE-TUNING. Stage 5 trains ResNet18 from ImageNet pretrained
# weights on a COMBINED dataset: dataset_v1 synth squares + 30
# manual-label real frames (games 8-11) + game4 PGN (184 frames) +
# game5 PGN (109 frames). 50/50 batch balancing via WeightedRandomSampler.
# game7 is the monitor and gates checkpoint selection.
# Test: games 2/6 (game4 and game5 are training data in stage 5).

Single-phase recipe (no Wölflein phase A/B):
  All params trainable from epoch 1. SGD(lr=1e-4, momentum=0.9, wd=1e-4).
  No scheduler.
Augmentation: same as stages 1/2/3/4 (color jitter @0.7, shear @0.8 ±8°,
...
Sampler: WeightedRandomSampler targeting 50% synth / 50% real per batch.
  ...
  With ~353 real frames (~22,600 squares), each real square cycles
  ~2.2× per epoch — much closer to balanced novelty vs. stage 4
  (where each of 1,920 real squares cycled ~26× per epoch).
Checkpoint selection: real_val_acc on game7. Early stop patience=8.

Stage 4 / 5 (Comparison B alongside stage 3). ImageNet pretrained
start (NOT v1 baseline). Single experimental variable vs. stage 3:
training procedure (joint-combined vs sequential FT).

Test partition (identical to stage 3):
...
# NO PRETRAINED_CKPT. Stage 5 starts from ImageNet weights via torchvision
...
HELD_OUT_GAMES = [2, 6]            # NOT [2, 4, 5, 6] — game4/5 are training data
...
# === Experiment dirs ===
EXP_DIR = f"{PROJECT_ROOT}/stage5_combined_323"
...
SYNTH_MONITOR_FRAC = 0.05         # 5% slice of dataset_v1, same as stages 1/2/3/4

NUM_WORKERS_SYNTH = 4
NUM_WORKERS_REAL = 4

# === Augmentation (same as stages 1/2/3/4) ===
...
print(f"held-out games:     {HELD_OUT_GAMES}  (game4/5 are training data, not test)")
...
# %% [Cell 3 — Load manual labels (30 — manual side of joint training)]
...
# Stage 5 uses ALL 30 manual labels (no subsetting) as the MANUAL part of
# the REAL side of joint training. game4 + game5 PGN frames (Cell 5) are the
# rest of the real side. The SYNTH side is the full dataset_v1 manifest.
STAGE5_MANUAL_N = 30
picked_rows = []
...
stage5_manual_df = pd.DataFrame(picked_rows).reset_index(drop=True)
assert len(stage5_manual_df) == STAGE5_MANUAL_N, (
    f"selected {len(stage5_manual_df)} manual images, expected {STAGE5_MANUAL_N} "
    f"(all manual labels). Check data/real_labels.csv row count."
)
games_covered = set(stage5_manual_df["game"].unique())
assert games_covered == {"game8", "game9", "game10", "game11"}, (
    f"all 4 games must be represented; got {games_covered}"
)

stage5_manual_path = f"{RESULTS_DIR}/stage5_manual_manifest.csv"
stage5_manual_df.to_csv(stage5_manual_path, index=False)
print(f"\nSelected stage-5 MANUAL training manifest (n={len(stage5_manual_df)}):")
print(stage5_manual_df[["game", "ply", "image_name", "fen"]].to_string(index=False))
print(f"\nwrote {stage5_manual_path}")
print(f"per-game picks: {stage5_manual_df['game'].value_counts().sort_index().to_dict()}")
...
manual_train_dataset = ManualLabelsDataset(
    stage5_manual_df, REAL_IMAGES_ROOT, transform=train_transform,
)
...
# --- Stage 5: synth TRAINING dataset (FULL manifest, with aug).
...
# %% [Cell 5 — RealGameDataset (game7 + game4 + game5) + train ConcatDataset]
...
# --- Stage 5: game4 + game5 PGN training datasets (additional real data).
# game4 has 184 frames, game5 has 109 frames. They are part of the
# REAL side of the joint training ConcatDataset, alongside the 30
# manual labels. game4 and game5 are excluded from the held-out test
# partition (HELD_OUT_GAMES = [2, 6]) for that reason.
game4_pgn_dataset = RealGameDataset(
    f"{PROJECT_ROOT}/data/game4_per_frame/gt.csv",
    f"{PROJECT_ROOT}/data/game4_per_frame/images",
    game_name="game4",
    transform=train_transform,   # WITH augmentation, like manual_train_dataset
)
print(f"game4_pgn_dataset: {len(game4_pgn_dataset):,} samples "
      f"({game4_pgn_dataset.manifest['image_name'].nunique()} frames × 64, "
      f"with train_transform aug)")

game5_pgn_dataset = RealGameDataset(
    f"{PROJECT_ROOT}/data/game5_per_frame/gt.csv",
    f"{PROJECT_ROOT}/data/game5_per_frame/images",
    game_name="game5",
    transform=train_transform,
)
print(f"game5_pgn_dataset: {len(game5_pgn_dataset):,} samples "
      f"({game5_pgn_dataset.manifest['image_name'].nunique()} frames × 64, "
      f"with train_transform aug)")

# --- Stage 5: train_dataset = synth + (manual + game4 PGN + game5 PGN).
# WeightedRandomSampler in Cell 6 balances this at 50/50 synth/real per
# batch. All four datasets retain their own __getitem__ contracts
# (with augmentation); the sampler just decides which index to draw.
real_train_dataset = ConcatDataset([
    manual_train_dataset, game4_pgn_dataset, game5_pgn_dataset,
])
...
print(f"\ntrain_dataset (synth + real concat): "
      f"{len(train_dataset):,} samples total")
print(f"  synth (full v1):  {n_synth:,} samples")
print(f"  real (manual + game4 + game5 PGN): {n_real:,} samples")
print(f"    manual: {len(manual_train_dataset):,}  "
      f"game4: {len(game4_pgn_dataset):,}  "
      f"game5: {len(game5_pgn_dataset):,}")
print(f"  synth fraction: {n_synth/(n_synth + n_real):.4f}")
print(f"  real  fraction: {n_real/(n_synth + n_real):.4f}")
print(f"  natural ratio is ~{n_synth/n_real:.0f}:1 synth:real")
print(f"  Sampler will compensate to 50/50 per batch.")
print(f"  Each real square cycles ~{NUM_SAMPLES_PER_EPOCH * 0.5 / n_real:.1f}"
      f"× per epoch (vs. stage 4's ~26×).")

print("\033[92m✓ Cell 5 — RealGameDataset (game7) + game4/game5 PGN + train concat — OK\033[0m")
...
# synth_train_dataset occupies indices [0, n_synth)
# real_train_dataset occupies indices [n_synth, n_synth + n_real)
...
print(f"  per-real-sample cycles per epoch ≈ "
      f"{NUM_SAMPLES_PER_EPOCH * 0.5 / n_real:.2f}")
print(f"  per-synth-sample cycles per epoch ≈ "
      f"{NUM_SAMPLES_PER_EPOCH * 0.5 / n_synth:.4f}")
...
def build_model():
    ...
    Stage 5 starts from ImageNet — NOT v1 baseline.
    """
...
# NOTE: stage 5 has NO phase A / phase B split. ImageNet weights are
...
idx = 0
assert idx < n_synth, "idx 0 must land in the synth portion of train_dataset"
s1, _ = train_dataset[idx]
s2, _ = train_dataset[idx]
arr1 = s1.float().numpy()
arr2 = s2.float().numpy()
same_sample_diff = float(np.abs(arr1 - arr2).mean())
print(f"  mean |s1 - s2| reading train_dataset[{idx}] (synth sample) "
      f"twice: {same_sample_diff:.4f}")
assert same_sample_diff > 0.01, (
    f"synth augmentations not firing — mean abs diff {same_sample_diff:.4f}"
)

# [1b-extra] Confirm real samples from EACH of the three real sources
# (manual, game4, game5) are augmented. Stage 5 stacks them in this
# order inside real_train_dataset, which is concat-position 1 in
# train_dataset (synth is position 0).
print("\n[1b-extra] Augmentation-firing check (real samples from each source):")
real_start = n_synth
manual_idx = real_start + 5                                          # 6th manual sample
game4_idx  = real_start + len(manual_train_dataset) + 50             # 51st game4 sample
game5_idx  = real_start + len(manual_train_dataset) + len(game4_pgn_dataset) + 20  # 21st game5 sample

for label, idx in [("manual", manual_idx), ("game4", game4_idx), ("game5", game5_idx)]:
    s1, _ = train_dataset[idx]
    s2, _ = train_dataset[idx]
    diff = float(np.abs(s1.numpy() - s2.numpy()).mean())
    print(f"  mean |s1 - s2| at idx {idx} ({label}) twice: {diff:.4f}")
    assert diff > 0.01, (
        f"{label} samples not augmented — mean abs diff {diff:.4f}. "
        f"Did the dataset receive transform=train_transform?"
    )
...
n_synth_drawn = sum(1 for i in sample_indices if i < n_synth)
...
plt.suptitle(
    f"Stage-5 augmented training crops (synth + real mix) "
...
# (~6,100) and game4/game5 PGN frames (300+) are skipped here.
print("\n[1c-b] Stage5 manual-frames sample plot (warped boards + labels):")
fig, axes = plt.subplots(5, 6, figsize=(24, 20))
for ax, (_, row) in zip(axes.ravel(), stage5_manual_df.iterrows()):
    ...
    ax.set_xticks([]); ax.set_yticks([])
plt.suptitle(
    "Stage5 training set (manual side only) — 30 manual-label frames with "
    "FEN labels overlaid. game4/game5 PGN frames are part of training but "
    "not plotted here (300+ frames).",
    fontsize=12,
)
plt.tight_layout(rect=(0, 0, 1, 0.96))
train_samples_path = f"{PLOTS_DIR}/stage5_manual_samples.png"
plt.savefig(train_samples_path, dpi=120)
plt.close()
print(f"  wrote {train_samples_path}")

# Stage 5 has no [1d] pretrained-weights check — we start from ImageNet
...
# be roughly chance (~13% for 13-class). The forgetting Δ vs. stage 3 still
...
# [4c] Single-phase parameter check (all params trainable in stage 5).
all_trainable = all(p.requires_grad for p in model.parameters())
print(f"\n[4c] All params trainable: {all_trainable}")
assert all_trainable, "stage 5 has no freeze phase; all params must be trainable"
...
print("\n" + "=" * 64)
print("Stage 5 smoke test passed. Ready for training.")
print("=" * 64)
...
# Stage 5 starts from ImageNet pretrained weights built in Cell 7.
...
print(f"[Cell 11 reset] rebuilt ImageNet model + fresh SGD(lr={LR}). Stage 5.")
...
for epoch in range(1, NUM_EPOCHS + 1):
    # Stage 5: single phase, all params trainable from epoch 1.
...
ax1.set_title("Accuracy — Stage 5 joint synth+real, single phase")
...
ax2.set_title("Loss — Stage 5 joint synth+real, single phase")
...
fig.suptitle(
    ...
    "(red dotted horiz = 0%; stage 5 is single-phase)",
    fontsize=12,
)
...
# %% [Cell 17 — Held-out games: dataset + loader for games 2/6 (stage 5 partition; game4/5 are training)]
real_test_datasets = {}
real_test_loaders = {}
...
# %% [Cell 19 — Aggregate across held-out games (games 2/6 — stage 5 partition; subset of stage 2's full held-out)]
...
plot_confusion_matrix(
    cm_c,
    f"Held-out aggregate (games 2/6 — stage 5 partition; game4/5 are training) — per-square acc={combined_per_square_acc:.4f}",
...
# %% [Cell 21 — Summary.md]
# Stage 5 starts from ImageNet — there are no SRC_* baseline-checkpoint
...
# Headline verdict for stage 5 — direct comparison to stage 3 (same partition,
# same real data: 30 manual + game4 + game5). Built after we load stage 3
# numbers below.

# --- Stage 3 comparison block (inserted into summary BEFORE the v1 per-class
# delta block). Stage 5 and stage 3 use the SAME test partition (games 2/6)
# and the SAME real data (30 manual + game4 + game5 PGN), so this comparison
# is DIRECT and valid — no caveat. Variable name `stage1_compare_lines` /
# `s1_paths` is kept from template lineage; the *contents* now refer to
# stage 3 paths and the headline comparison is stage 3 (sequential FT) vs.
# stage 5 (joint).
stage1_compare_lines = [
    "## Comparison to stage3_323 (same real data, same test partition — Comparison B)",
    "",
    "Same real data (30 manual + game4 + game5 PGN), same test set (games "
    "2/6), identical augmentation. The single axis varied is the **training "
    "procedure**: stage 3 fine-tunes the v1 synth-trained baseline on real "
    "data only (sequential), whereas stage 5 trains jointly on synth+real "
...
s1_paths = {
    "game7":     f"{PROJECT_ROOT}/fine_tuning/stage3_323/results/game7_results.json",
    "agg":       f"{PROJECT_ROOT}/fine_tuning/stage3_323/results/held_out_aggregate.json",
    "synth":     f"{PROJECT_ROOT}/fine_tuning/stage3_323/results/synth_test_results.json",
}
s1 = {}
...
# Per-class deltas vs. stage 3 on the same aggregate held-out partition.
if "agg" in s1 and "per_class_acc" in s1["agg"]:
    ...
    for cls in range(NUM_CLASSES):
        short = CLASS_SHORT[cls]
        s3_acc = s1["agg"]["per_class_acc"].get(short)   # stage 3 (sequential FT)
        s5_acc = combined_per_class[cls]                 # stage 5 (joint, this run)
        if s3_acc is None or s3_acc != s3_acc:
            stage1_compare_lines.append(
                f"| {short} | n/a | "
                f"{s5_acc if s5_acc == s5_acc else float('nan'):.4f} | n/a |"
            )
        else:
            delta = s5_acc - s3_acc if s5_acc == s5_acc else float("nan")
            stage1_compare_lines.append(
                f"| {short} | {s3_acc:.4f} | "
                f"{s5_acc if s5_acc == s5_acc else float('nan'):.4f} | "
                f"{delta:+.4f} |"
            )
    stage1_compare_lines.append("")

# Headline verdict — now that stage 3 numbers are loaded.
agg_s3_ref = s1.get("agg", {}).get("per_square_acc")
if agg_s3_ref is not None:
    beat = combined_per_square_acc > agg_s3_ref
    verdict = (
        f"**Verdict:** stage5 (joint training) {'BEAT' if beat else 'did NOT beat'} "
        f"stage 3 (sequential FT) on the matched held-out partition (games "
        f"2/6): {combined_per_square_acc:.4f} vs. {agg_s3_ref:.4f} "
        f"per-square (Δ = {combined_per_square_acc - agg_s3_ref:+.4f})."
    )
else:
    verdict = (
        f"**Verdict:** stage5 per-square acc on held-out (games 2/6) is "
        f"**{combined_per_square_acc:.4f}**. stage 3 held-out aggregate not "
        f"available on disk — re-run stage 3 to populate "
        f"`fine_tuning/stage3_323/results/held_out_aggregate.json` for the "
...
summary_lines = [
    ...
    "30 manual labels + game4 PGN (184 frames) + game5 PGN (109 frames). "
    "Total real: ~353 frames / ~22,600 squares. Combined via two-level "
    "ConcatDataset (synth + (manual + game4 + game5)).",
    ...
    f"Each real square cycles ~{NUM_SAMPLES_PER_EPOCH * 0.5 / n_real:.1f}× per epoch "
    f"(vs. stage 4's ~26×).",
    ...
    "",
    "NOTE: stage 5 starts from ImageNet, so 'BEFORE training' is ~chance "
    ...
    "where it falls. With ~10× more real data than stage 4, watch whether "
    "the larger real pool erodes this number relative to stage 4.",
    "",
    ...
    *stage1_compare_lines,
    ...
    "",
    "Stage 5 starts from ImageNet (not v1 baseline), so the question for "
...
# %% [Cell 22 — Stage 3 re-eval on games 2/6 (same-partition direct comparison)]
# NOTE: stage 5 and stage 3 already share the SAME test partition (games
# 2/6) and the SAME real data, so this re-eval reproduces stage 3's
# published numbers exactly. It's partly redundant, but acts as a sanity
# check that the test pipeline is identical between scripts (and writes a
# JSON the report tooling can read uniformly).
print("=" * 64)
print("Loading stage_3 best_real checkpoint and re-evaluating on the")
print("stage 5 test partition (games 2/6) for direct comparison.")
print("=" * 64)

stage3_ckpt_path = f"{PROJECT_ROOT}/fine_tuning/stage3_323/checkpoints/best_real.pt"
if not Path(stage3_ckpt_path).exists():
    print(f"[Cell 22] stage 3 checkpoint not found: {stage3_ckpt_path}")
    print(f"[Cell 22] skipping re-eval. Stage 5 summary.md will not have"
          f" the bridge row.")
else:
    ...
    # but we overwrite its weights with stage 3's checkpoint immediately
    # — so the ImageNet init is just scaffold.
    stage3_model = build_model().to(DEVICE)
    stage3_model.load_state_dict(stage3_ckpt["model_state_dict"])
    stage3_model.eval()
    print(f"loaded stage_3 best_real ckpt (epoch {stage3_ckpt['epoch']}, "
          f"real_val_acc={stage3_ckpt['real_val_acc']:.4f})")

    # Re-evaluate on the SAME loaders we just built (games 2/6).
    s3_per_game_acc = {}
    s3_combined_preds = []
    s3_combined_labels = []
    s3_combined_manifest = []
    for N, loader in real_test_loaders.items():
        _, g_acc, g_preds, g_labels = evaluate(
            stage3_model, loader, criterion, DEVICE, f"stage3_on_game{N}")
        s3_per_game_acc[N] = g_acc
        s3_combined_preds.append(g_preds)
        s3_combined_labels.append(g_labels)
        mf = real_test_datasets[N].manifest.copy()
        mf["pred"] = g_preds; mf["game_num"] = N
        s3_combined_manifest.append(mf)
        print(f"  stage3 on game{N}: per-sq={g_acc:.4f}")

    s3_combined_preds = np.concatenate(s3_combined_preds)
    s3_combined_labels = np.concatenate(s3_combined_labels)
    s3_combined_manifest = pd.concat(s3_combined_manifest, ignore_index=True)

    s3_combined_per_sq = float((s3_combined_preds == s3_combined_labels).mean())
    s3_piece_mask = (s3_combined_labels != 12)
    s3_combined_piece = float(
        (s3_combined_preds[s3_piece_mask] == s3_combined_labels[s3_piece_mask]).mean()
    ) if s3_piece_mask.any() else float("nan")
    s3_per_class = per_class_accuracy(
        s3_combined_preds, s3_combined_labels, NUM_CLASSES)

    # Per-board accuracy for stage 3 on games 2/6.
    s3_per_board = (
        s3_combined_manifest.assign(correct=lambda d: d["pred"] == d["label"])
        .groupby(["game_num", "image_name"])["correct"]
        .agg(["sum", "count"])
    )
    s3_per_board["all_correct"] = s3_per_board["sum"] == s3_per_board["count"]
    s3_n_boards = len(s3_per_board)
    s3_n_all_correct = int(s3_per_board["all_correct"].sum())

    stage3_reeval = {
        "test_partition": ["game2", "game6"],
        "n_frames": int(s3_n_boards),
        "n_squares": int(len(s3_combined_preds)),
        "per_square_acc": s3_combined_per_sq,
        "per_board_acc": s3_n_all_correct / s3_n_boards if s3_n_boards else 0.0,
        "n_all_correct": int(s3_n_all_correct),
        "piece_only_acc": s3_combined_piece,
        "per_class_acc": {
            CLASS_SHORT[c]: s3_per_class[c] for c in range(NUM_CLASSES)
        },
    }
    Path(f"{RESULTS_DIR}/stage3_reeval_on_games_2_6.json").write_text(
        json.dumps(stage3_reeval, indent=2))
    print(f"\nstage 3 on games 2/6 partition:")
    print(f"  per-sq: {s3_combined_per_sq:.4f}")
    print(f"  piece-only: {s3_combined_piece:.4f}")
    print(f"  per-board: {s3_n_all_correct}/{s3_n_boards}="
          f"{s3_n_all_correct/max(s3_n_boards,1):.4f}")
    print(f"wrote {RESULTS_DIR}/stage3_reeval_on_games_2_6.json")
    ...
    bridge_lines = [
        ...
        "",
        "Stage 3's checkpoint evaluated on the EXACT same test set used by "
        "stage 5. Because both stages already share the games 2/6 partition, "
        "these numbers should reproduce stage 3's published results — this "
        "section is both a sanity check that the test pipeline is identical "
        "and a single-pane comparison row for the report.",
        "",
        "| metric | stage3_323 (sequential FT) | stage5_combined_323 (joint) | Δ |",
        "|--------|---------------------------:|----------------------------:|---:|",
        f"| per-sq acc | {s3_combined_per_sq:.4f} | "
        f"{combined_per_square_acc:.4f} | "
        f"{combined_per_square_acc - s3_combined_per_sq:+.4f} |",
        f"| piece-only | {s3_combined_piece:.4f} | "
        f"{combined_piece_acc:.4f} | "
        f"{combined_piece_acc - s3_combined_piece:+.4f} |",
        f"| per-board  | "
        f"{s3_n_all_correct/max(s3_n_boards,1):.4f} "
        f"({s3_n_all_correct}/{s3_n_boards}) | "
        f"{combined_per_board_acc:.4f} "
        f"({combined_n_all_correct}/{combined_n_boards}) | "
        f"{combined_per_board_acc - s3_n_all_correct/max(s3_n_boards,1):+.4f} |",
        "",
    ...
    for cls in range(NUM_CLASSES):
        s3 = s3_per_class[cls]
        s5 = combined_per_class[cls]
        if s3 == s3 and s5 == s5:
            bridge_lines.append(
                f"| {CLASS_SHORT[cls]} | {s3:.4f} | {s5:.4f} | "
                f"{s5 - s3:+.4f} |"
            )
        else:
            bridge_lines.append(
                f"| {CLASS_SHORT[cls]} | "
                f"{s3 if s3 == s3 else float('nan'):.4f} | "
                f"{s5 if s5 == s5 else float('nan'):.4f} | n/a |"
            )
    bridge_lines.append("")
...
print("\033[92m✓ Cell 22 — Stage 3 re-eval bridge — OK\033[0m")
