"""Automated sanity checks for the synthetic dataset.

Catches missing/truncated/wrong-sized images, CSV/disk drift, duplicate
renders, FENs with fewer than 3 camera views, and file-size outliers.
...
EXPECTED_CAMERAS_PER_FEN = 3  # 1_overhead + 2_west + 3_east (black view)

# Occupancy floor below which a FEN/image pair is almost certainly misaligned.
# Random pairings average ~33/64; the std-dev heuristic floors near 44/64 on
# correct pairings, so 40 sits between "heuristic noise" and "mis-paired".
...
def fen_to_occupancy(fen_first_field):
    """FEN piece-placement -> 8x8 bool occupancy grid; row 0 = rank 8, col 0 = file a."""
...
def image_to_occupancy(image_path, center_frac=0.5):
    """Estimate per-square occupancy from a rectified overhead image: per-square
    grayscale std over the central `center_frac` (excludes wood grain, keeps the
    piece base), then Otsu-thresholded into a binary occupancy mask."""
    ...
    # Otsu threshold over the 64 std values.
...
# All 8 dihedral (D4) transforms of an 8x8 grid.
DIHEDRAL = [
    ("id",             lambda g: g),
...
def main():
    ...
    # Every listed image exists on disk.
    ...
    # No orphan files on disk that aren't in the CSV.
    ...
    # Every PNG loads and has the expected dimensions.
    bad_dims = []
    unreadable = []
    for r in rows:
    ...
    # Each FEN has exactly 3 camera views.
    ...
    # All images unique (no duplicate renders).
    ...
    # File-size sanity (flag partial writes).
    tiny, huge = [], []
    for r in rows:
        p = DATASET_DIR / r["image_path"]
        if not p.exists():
            continue
        sz = p.stat().st_size
        if sz < 30 * 1024:            # suspicious for a 512x512 PNG
            tiny.append((p.name, sz))
        if sz > 3 * 1024 * 1024:       # far larger than expected
    ...
    # FEN <-> image occupancy: rendered pieces match the FEN under one
    ...
    per_image = []  # (row, scores_by_orient, error)
    ...
    # Expected orientation per view = the one that wins (or ties) most often;
    # symmetric positions tie, so credit all top-scoring orientations.
    ...
    # Inconsistent = expected orientation doesn't tie for top score (symmetric
    # positions tie across orientations and pass).
    inconsistent = []
    for r, sc, err in per_image:
        if not sc:
            continue
